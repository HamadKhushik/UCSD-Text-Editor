...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
		...
		// You can play around with this.
		int start = 50000;
		System.out.println("No Of Chars \t" + " Basic Document \t" + "Efficient Document \t\n");
				...
				numToCheck += increment)
		{
			...
			// time check for Basic Document
			long startTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				BasicDocument bDocument = new BasicDocument(testString);
				double fleschScore = bDocument.getFleschScore();
			}  
			long endTime = System.nanoTime();
			float timeTaken = (endTime - startTime) / 1000000;
			System.out.printf("\t\t"+timeTaken + "\t\t\t");

			// time check for Effiient Document
			startTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				EfficientDocument eDocument = new EfficientDocument(testString);
				double fleschScore = eDocument.getFleschScore();
			}
